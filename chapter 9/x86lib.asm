INCLUDE link.inc				; Last updated: 4.6.2018
include macros.inc
...
; 55. displayAsciiSpaced
; 56. getVowelRatio
; 57. isVowel
; 58. getSetsWithinVowelProbability
; 59. scanLetterArray
; 60. countVowels
; 61. str_concatL
; 62. getColumnsArray
; 63. getDiagonalsArray
; 64. scanLetterArrayDiagonals
; 65. randomAsciiArrayMax9
; 66. better_calc_row_sum
; 67. convertAsciiToIntger
; 68. displayIntegerTable
; 69. calc_row_sum
...
getVowelRatio proc   ptrArray324:ptr dword, leng42:dword
pushad
			mov esi, ptrArray324
			mov ecx, leng42
			mov ebx, 0
...
isVowel proc
; rec: eax = char, ret = cf=1 if character
		cmp al, 41h
		je yes
		cmp al, 45h
		je yes
...
getSetsWithinVowelProbability proc ptrCharArray:ptr dword, sizeLine:dword, ptrSetList:ptr dword, prob35:dword
pushad
				.data
				columnsArray byte 100h dup(0)
				diagonalsArray byte 100h dup(0)
				.code
				invoke scanLetterArray, ptrCharArray, sizeLine, ptrSetList, prob35
		
				invoke getColumnsArray, ptrCharArray, addr columnsArray, sizeLine
				invoke scanLetterArray, addr columnsArray, sizeLine, ptrSetList, prob35
			
				invoke getDiagonalsArray, ptrCharArray, addr diagonalsArray, sizeLine
				invoke scanLetterArrayDiagonals, addr diagonalsArray, sizeLine, ptrSetList, prob35


popad
ret
getSetsWithinVowelProbability endp
...
scanLetterArray proc	ptrCharArray1:ptr dword, sizeLine1:dword, ptrSetList1:ptr dword, prob351:dword
pushad
				mov esi, ptrCharArray1
				mov ecx, sizeLine1
			

			l1: invoke countVowels, esi, sizeLine1
				mov ebx, 100
				mul ebx
				div sizeLine1
				cmp eax, prob351
				je addLine
			l2: add esi, sizeLine1
				loop l1
				jmp ext

addLine:
			invoke str_concatL, ptrSetList1, esi, sizeLine1
...
countVowels proc  uses esi ebx ecx ptrarra1:ptr dword, leng132:dword
...
str_concatL proc ptrtarg3:ptr dword, ptrso3:ptr dword, maxLeng1:dword
	local targetLength:dword, sourceLength:dword
pushad
	...
	cld
	rep movsb		; mov [edi], [esi]         until ecx=0
	mov byte ptr [edi], 20h    ; add space
...
getColumnsArray proc    ptrCharArray32:ptr dword, ptrColArray:ptr dword, size399:dword
pushad
				mov esi, ptrCharArray32
				mov edi, ptrColArray
				mov ecx, size399
				mov ebx, 0
...
getDiagonalsArray proc ptrCharArray34:ptr dword, diagonalsArray7:ptr dword, sizeLine:dword
pushad
				mov esi, ptrCharArray34
				mov edi, diagonalsArray7
				mov ecx, sizeLine

			l1: mov al, byte ptr [esi]		; get left to right diagonal
				mov byte ptr [edi], al
				inc edi
				add esi, sizeLine
				add esi, 1
				...
				mov ecx, sizeLine			; get right to left diagonal
			    mov esi, ptrCharArray34
			l2: add esi, sizeLine
				sub esi, 1
				mov al, byte ptr [esi]
...
scanLetterArrayDiagonals proc diagonalsArray32:ptr dword, sizeLine41:dword, ptrSetList42:ptr dword, prob35:dword
pushad
				mov esi, diagonalsArray32
				mov ecx, 2
			

			l1: invoke countVowels, esi, sizeLine41
				mov ebx, 100
				mul ebx
				div sizeLine41
				cmp eax, prob35
				je addLine
			l2: add esi, sizeLine41
				loop l1
				jmp ext

addLine:
			invoke str_concatL, ptrSetList42, esi, sizeLine41
...
randomAsciiArrayMax9 proc ptrArray72:ptr dword, leng72:dword
pushad
			mov esi, ptrArray72
			mov ecx, leng72
...
Better_calc_row_sum proc uses ebx esi edx	    ptrArray73:ptr dword, width73:dword, type73:dword, rowIndex73:dword
; ret: eax = sum
				.data
				type2 byte 2
				.code
...
convertAsciiToInteger proc ptrArray77:ptr dword, leng77:dword
pushad
			mov esi, ptrArray77
			mov ecx, leng77

		l1: and byte ptr [esi], 0fh
			inc esi
			loop l1
popad
ret
convertAsciiToInteger endp


; 68

displayIntegerTable proc ptrArray78:ptr dword, widthrows7:dword, type78:dword, numRows78:dword
pushad
			mov esi, ptrArray78
			mov eax, widthrows7
			mov edx, 0
...
; ret: eax = sum
				...
				push ebp
				mov ebp, esp
				ptrArray73 equ [ebp + 8]
				width73 equ [ebp + 12]
				type73 equ [ebp + 16]
				rowIndex73 equ [ebp + 20]
